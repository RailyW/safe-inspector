...
- `config.go`：定义 `Config`、`SSHTarget`、`DBTarget`、模板结构、secret 结构和 `Store` 读写方法。
- `config_test.go`：覆盖初始化文件布局、配置读取、secret 解密和错误主秘钥拒绝。

## 文件职责

- `config.yaml`：保存 SSH/数据库目标、允许模板、参数约束、超时、输出限制等非敏感策略。
